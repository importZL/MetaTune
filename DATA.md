...
* **Experimental splits**: The test set (159 images, `splits/bccd/test.txt`) is fixed across all runs. The four training images are drawn from the dataset’s original training partition. `splits/bccd/run_1/` lists the exact images of one of the three runs averaged in Figure 2, including their assignment to the non-meta subset \(D_1\) (`non_meta_D1.txt`) and the meta subset \(D_2\) (`meta_D2.txt`). Pass it to `train.sh` via `SPLIT_DIR=splits/bccd/run_1`.
...
* **Exact experimental split**: see `splits/bccd/run_1/` and `splits/bccd/test.txt`
