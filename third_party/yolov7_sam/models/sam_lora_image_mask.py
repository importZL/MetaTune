...
class LoRA_Sam(nn.Module):
    ...
    def save_lora_parameters(self, filename: str) -> None:
        ...
        for key, value in state_dict.items():
            if 'prompt_encoder' in key and 'no_mask_embed' not in key:
    ...
    def load_lora_parameters(self, filename: str, device: torch.device) -> None:
        ...
        # load prompt encoder
        prompt_encoder_keys = [k for k in sam_keys if ('prompt_encoder' in k and 'no_mask_embed' not in k)]
    ...
    def forward(self, batched_input, multimask_output, image_size, point=None, bbox=None, mask=None):
