...
class Detect(nn.Module):
    ...
    def forward(self, x, check_anchor=False):
        z = []  # inference output
...
class Segment(Detect):
    ...
    def forward(self, x, check_anchor):
        p = self.proto(x[0])
        x = self.detect(self, x)
        if not check_anchor:
            return (x[0], p) if self.export else (x[0], (x[1], p))
        else:
            return (x, p)
...
class ISegment(IDetect):
    ...
    def forward(self, x, check_anchor):
        p = self.proto(x[0])
        x = self.detect(self, x, check_anchor)
        if not check_anchor:
            return (x[0], p) if self.export else (x[0], (x[1], p))
        else:
            return (x, p)
...
class IRSegment(IDetect):
    ...
    def forward(self, x, check_anchor):
        ...
        x = self.detect(self, x[self.nl:], check_anchor)
        if not check_anchor:
            return (x[0], p) if self.export else (x[0], (x[1], p))
        else:
            return (x, p)
...
class BaseModel(nn.Module):
    ...
    def _forward_once(self, x, check_anchor=False, profile=False, visualize=False):
        y, dt = [], []  # outputs
        for m in self.model:
            ...
            if isinstance(m, (Segment, ISegment, IRSegment)):
                x = m(x, check_anchor)  # run
            else:
                x = m(x)  # run
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None, opt=None):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, IDetect, Segment, ISegment, IRSegment)):
            ...
            forward = lambda x: self.forward(x, check_anchor=True)[0] if isinstance(m, (Segment, ISegment, IRSegment)) else self.forward(x)
    ...
    def forward(self, x, check_anchor=False, augment=False, profile=False, visualize=False):
        if augment:
            return self._forward_augment(x)  # augmented inference, None
        return self._forward_once(x, check_anchor, profile, visualize)  # single-scale inference, train
...
class SegmentationModel(DetectionModel):
    ...
    def __init__(self, cfg='yolov5s-seg.yaml', ch=3, nc=None, anchors=None, opt=None):
        super().__init__(cfg, ch, nc, anchors, opt)
