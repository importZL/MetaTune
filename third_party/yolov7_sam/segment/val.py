...
from utils.plots import output_to_target, plot_val_study, output_to_target_sam
...
def process_batch(detections, labels, iouv, pred_masks=None, gt_masks=None, overlap=False, masks=False):
    ...
    if masks:
        ...
        if gt_masks.shape[1:] != pred_masks.shape[1:]:
            gt_masks = F.interpolate(gt_masks[None], pred_masks.shape[1:], mode="nearest")[0]
...
def run(
    ...
    s = ('%20s' + '%10s' * 12) % ('Class', 'Images', 'Instances', 'Box(P', "R", "mAP50", "mAP75", "mAP50-95)", "Mask(P", "R",
    ...
    for batch_i, (im, targets, paths, shapes, masks) in enumerate(pbar):
        ...
        for si, pred in enumerate(out):
            ...
            sam_bboxes, scores, pred_labels = output_to_target_sam([pred], max_det=-1, thre=-1)
            sam_output = sam(im.float(), multimask_output=False, image_size=imgsz, bbox=sam_bboxes)
            # pred_masks_logits, pred_masks1 = torch.max(sam_output['low_res_logits'], dim=1)
            pred_masks = (sam_output['low_res_logits'] > 0).squeeze().float()
    ...
    # Print results
    pf = '%20s' + '%10i' * 2 + '%10.3g' * 10  # print format
    ...
    mp_bbox, mr_bbox, map50_bbox, map75_bbox, map_bbox, mp_mask, mr_mask, map50_mask, map75_mask, map_mask = metrics.mean_results()
    ...
    final_metric = mp_bbox, mr_bbox, map50_bbox, map75_bbox, map_bbox, mp_mask, mr_mask, map50_mask, map75_mask, map_mask
