...
def check_amp(model):
    ...
    im = np.ones((640, 640, 3)) # f if f.exists() else 'https://ultralytics.com/images/bus.jpg' if check_online() else np.ones((640, 640, 3))
    try:
...
def clip_coords(boxes, shape):
    ...
    if isinstance(boxes, torch.Tensor):  # faster individually
        ...
        boxes[:, 0] = boxes[:, 0] * (boxes[:, 0] >= 0).type_as(boxes[:, 0]) * (boxes[:, 0] <= shape[1]).type_as(boxes[:, 0])
        boxes[:, 1] = boxes[:, 1] * (boxes[:, 1] >= 0).type_as(boxes[:, 1]) * (boxes[:, 1] <= shape[0]).type_as(boxes[:, 1])
        boxes[:, 2] = boxes[:, 2] * (boxes[:, 2] >= 0).type_as(boxes[:, 2]) * (boxes[:, 2] <= shape[1]).type_as(boxes[:, 2])
        boxes[:, 3] = boxes[:, 3] * (boxes[:, 3] >= 0).type_as(boxes[:, 3]) * (boxes[:, 3] <= shape[0]).type_as(boxes[:, 3])
    else:  # np.array (faster grouped)
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Compute conf
        x_clone = x[:, 5:].clone()
        x_clone = x_clone * x[:, 4:5]
        x = torch.cat((x[:, :5], x_clone), dim=1)
