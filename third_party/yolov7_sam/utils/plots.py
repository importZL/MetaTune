...
class Annotator:
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                _l, _t, _r, _b = self.font.getbbox(label); w, h = _r - _l, _b - _t  # PIL>=10
    ...
    def text(self, xy, text, txt_color=(255, 255, 255), anchor='top'):
        ...
        if anchor == 'bottom':  # start y from font bottom
            _l, _t, _r, _b = self.font.getbbox(text); w, h = _r - _l, _b - _t  # PIL>=10
            xy[1] += 1 - h
...
def output_to_target_sam(output, max_det=300, thre=-1):
    targets = []
    for i, o in enumerate(output):
        if max_det > 0:
            box, conf, cls = o[:max_det, :6].cpu().split((4, 1, 1), 1)
        else:
            box, conf, cls = o[:, :6].cpu().split((4, 1, 1), 1)
        j = torch.full((conf.shape[0], 1), i)
        targets.append(torch.cat((j, cls, xyxy2xywh(box), conf), 1))
    out = torch.cat(targets, 0) # [batch_id, class_id, x, y, w, h, conf]
    if thre:
        idx = out[:, -1] > thre
        bboxes = out[idx, 2:6]
        scores = out[idx, -1]   # use concatenated `out`, not last-batch `conf`
        labels = out[idx, 1]
    else:
        bboxes = out[:, 2:6]
        scores = out[:, -1]
        labels = out[:, 1]
    sam_bboxes = bboxes.clone().cuda()
    scores = scores.cuda()
    labels = labels.cuda()
    marea = bboxes[:, 2:3].prod(1)
    sam_bboxes[:, 0] = bboxes[:, 0] - bboxes[:, 2] / 2  # top left x
    sam_bboxes[:, 1] = bboxes[:, 1] - bboxes[:, 3] / 2  # top left y
    sam_bboxes[:, 2] = bboxes[:, 0] + bboxes[:, 2] / 2  # bottom right x
    sam_bboxes[:, 3] = bboxes[:, 1] + bboxes[:, 3] / 2  # bottom right y
    return sam_bboxes, scores, labels
