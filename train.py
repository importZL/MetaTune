...
parser.add_argument('--swap_meta', action='store_true',
                    help='If set, swap meta/non-meta assignment: prompt embedding becomes the lower-level (non-meta) variable trained on D1; LoRA+decoder become the upper-level (meta) variable trained on D2. Only valid with 1st-order (--unrolled=False).')
...
if __name__ == "__main__":
    ...
    if args.swap_meta:
        assert not args.unrolled, "--swap_meta is only supported with 1st-order (omit --unrolled)"
