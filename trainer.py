...
def trainer(args, model, snapshot_path, multimask_output, low_res):
    ...
    logger = wandb.init(project='MetaTune', name=exp_name, resume='allow', anonymous='must', mode=args.wandb_mode)
    ...
    # train_set, valid_set = random_split(db_train, [3, 1])
    # trainloader = DataLoader(train_set, batch_size=args.batch_size, num_workers=0, pin_memory=True,
    #                          worker_init_fn=worker_init_fn)
    # validloader = DataLoader(valid_set, batch_size=args.batch_size, num_workers=0, pin_memory=True,
    #                          worker_init_fn=worker_init_fn)

    trainloader = DataLoader(db_train, batch_size=args.batch_size, num_workers=0, pin_memory=True,
                             worker_init_fn=worker_init_fn, sampler=selector[:num_train])
    validloader = DataLoader(db_train, batch_size=args.batch_size, num_workers=0, pin_memory=True,
                             worker_init_fn=worker_init_fn, sampler=selector[num_train:])
    ...
    if getattr(args, 'swap_meta', False):
        main_params = [p for n, p in model.named_parameters()
                       if p.requires_grad and ("no_mask_embed" in n)]
    else:
        main_params = [p for n, p in model.named_parameters()
                       if p.requires_grad and ("no_mask_embed" not in n)]
    optimizer = optim.AdamW(
        main_params,
        lr=base_lr, betas=(0.9, 0.999), weight_decay=args.weight_decay)
    
    # db_test = Synapse_dataset(train_dir=args.root_path.replace('/train', '/test'), 
    ...
    for epoch_num in range(max_epoch):
        ...
        # train_score = validate(args, model, trainloader, multimask_output)
    ...
    # f = open(f"./output_other/result_ours_{args.dataset}.txt", "w")
